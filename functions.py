...
def detects_faces(img, detector_backend = 'opencv', grayscale = False, enforce_detection = True):
...
def detects_faces2(img, detector_backend = 'mtcnn', grayscale = False, enforce_detection = True):
          #      img2=preprocess_face(img, target_size=(224, 224), grayscale = False, enforce_detection = True, detector_backend = 'opencv')
                if not "face_detector" in globals():
                   initialize_detector(detector_backend = detector_backend)
                img = load_image(img)
           #     face_detector = MTCNN()
...
def detects_multi (img, detector_backend = 'mtcnn', grayscale = False, enforce_detection = True):
          #      img2=preprocess_face(img, target_size=(224, 224), grayscale = False, enforce_detection = True, detector_backend = 'opencv')
                if not "face_detector" in globals():
                   initialize_detector(detector_backend = detector_backend)
                img = load_image(img)
...
def last_detect(img, detector_backend = 'ssd', grayscale = False, enforce_detection = True):
                if not "face_detector" in globals():
                   initialize_detector(detector_backend = detector_backend)
                ssd_labels = ["img_id", "is_face", "confidence", "left", "top", "right", "bottom"]
                img = load_image(img)            
